...
export async function POST(request: NextRequest) {
  try {
    const { seferId, sirketten_alinan_ucret } = await request.json();

    // Parametrelerin doğruluğunu kontrol et
    if (!seferId || sirketten_alinan_ucret === undefined) {
    ...
    const ucret = parseFloat(sirketten_alinan_ucret.toString());
