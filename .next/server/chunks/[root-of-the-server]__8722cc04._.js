...
"[project]/.next-internal/server/app/api/eski-veri-yukle/route/actions.js [app-rsc] (server actions loader, ecmascript)": (function(__turbopack_context__) {
...
"[project]/src/app/api/eski-veri-yukle/route.ts [app-route] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    try {
        ...
        console.log('Eski veri yükleme isteği:', {
            ay,
            donem,
            seferSayisi: Object.keys(seferler).length
        });
        ...
        for (const [sirketIdStr, sirketSeferleri] of Object.entries(seferler)){
            ...
            for (const sefer of sirketSeferleri){
                try {
                    ...
                    console.log(`Sefer işleniyor: Şirket ${sirketId}, Sefer ${seferNo}`, sefer.tumVeriler ? 'Veri mevcut' : 'Veri yok');
                    ...
                    if (soforAdi && soforAdi.trim() !== '') {
                        ...
                        try {
                            ...
                            console.log(`Şirket ${sirketId} için tüm şöförler:`, allAraclar.map((a)=>`ID: ${a.arac_id}, Ad: ${a.sofor_adi}`).join(', '));
                            // Önce birebir eşleşme ara (case insensitive)
                            const exactMatch = allAraclar.find((a)=>a.sofor_adi.toLowerCase().trim() === soforAdi.toLowerCase().trim());
                            if (exactMatch) {
                                ...
                                if (!isNaN(extractedId)) {
                                    ...
                                    try {
                                        ...
                                        if (aracById) {
                                            ...
                                            const cleanSoforAdi = soforAdi.replace(/[\(\[\{].*?[\)\]\}]/g, '').replace(/[\-–]\s*\d+\s*$/g, '').replace(/\s*ID:?\s*\d+/gi, '').replace(/#\s*\d+/g, '').trim();
                                            if (cleanSoforAdi) {
                                                try {
                                                    ...
                                                    if (!mevcutArac) {
                                                        ...
                                                        const yeniArac = await prisma.araclar.create({
                                                            data: {
                                                                plaka: `PLAKA-${extractedId}`,
                                                                sofor_adi: cleanSoforAdi,
                                                                vergi_numarasi: '00000000000',
                                                                sirket_id: sirketId
                                                            }
                                                        });
                        ...
                        if (!aracId) {
                            ...
                            const cleanSoforAdi = soforAdi.replace(/[\(\[\{].*?[\)\]\}]/g, '') // Parantez içini sil
                            ...
                            if (cleanSoforAdi) {
                                try {
                                    ...
                                    const normalizeName = (name)=>{
                                        return name.split('').map((c)=>turkishChars[c] || c).join('');
                                    };
                                    ...
                                    let eslesenArac = tum_araclar.find((a)=>a.sofor_adi.toLowerCase().trim() === cleanSoforAdi.toLowerCase().trim());
                                    // Tam eşleşme yoksa, contains ile arayalım
                                    if (!eslesenArac) {
                                        eslesenArac = tum_araclar.find((a)=>a.sofor_adi.toLowerCase().includes(cleanSoforAdi.toLowerCase()) || cleanSoforAdi.toLowerCase().includes(a.sofor_adi.toLowerCase()));
                                    }
                                    ...
                                    if (!eslesenArac) {
                                        ...
                                        eslesenArac = tum_araclar.find((a)=>{
                                            ...
                                            return normalizedDbName === normalizedSoforAdi || normalizedDbName.includes(normalizedSoforAdi) || normalizedSoforAdi.includes(normalizedDbName);
                                        });
                                    }
                                    if (eslesenArac) {
                                        ...
                                        let eslesenAracTumSirketler = tumSirketlerAraclar.find((a)=>a.sofor_adi.toLowerCase().trim() === cleanSoforAdi.toLowerCase().trim());
                                        // Tam eşleşme yoksa, contains ile arayalım
                                        if (!eslesenAracTumSirketler) {
                                            eslesenAracTumSirketler = tumSirketlerAraclar.find((a)=>a.sofor_adi.toLowerCase().includes(cleanSoforAdi.toLowerCase()) || cleanSoforAdi.toLowerCase().includes(a.sofor_adi.toLowerCase()));
                                        }
                                        // Türkçe karakter dönüşümlü arama
                                        if (!eslesenAracTumSirketler) {
                                            const normalizedSoforAdi = normalizeName(cleanSoforAdi.toLowerCase());
...
function getExcelValue(sefer, columnName) {
    if (!sefer.tumVeriler) return null;
    // Şöför için özel arama
    if (columnName === 'Şöför') {
        console.log('>>> Şöför değeri aranıyor...');
        // Önce tam eşleşme ara
        if (sefer.tumVeriler[columnName] !== undefined && sefer.tumVeriler[columnName] !== null && sefer.tumVeriler[columnName] !== '') {
        ...
        const soforKeys = Object.keys(sefer.tumVeriler).filter((key)=>{
            ...
            return lowerKey.includes('şöför') || lowerKey.includes('şoför') || lowerKey.includes('soför') || lowerKey.includes('sofor') || lowerKey.includes('driver') || lowerKey.includes('sürücü');
        });
    ...
    const matchingKey = Object.keys(sefer.tumVeriler).find((key)=>key.toLowerCase().includes(columnName.toLowerCase()) || columnName.toLowerCase().includes(key.toLowerCase()));
    if (matchingKey && sefer.tumVeriler[matchingKey] !== null && sefer.tumVeriler[matchingKey] !== undefined && sefer.tumVeriler[matchingKey] !== '') {
        return sefer.tumVeriler[matchingKey].toString();
    }
    return null;
    ...
    try {
        ...
        if (cleanDateStr.includes(' ') && !cleanDateStr.includes('.') && !cleanDateStr.includes('/') && !cleanDateStr.includes('-')) {
            ...
            if (parts.length >= 3) {
                ...
                if (!isNaN(gun) && !isNaN(ay) && !isNaN(yil) && gun >= 1 && gun <= 31 && ay >= 1 && ay <= 12) {
...
function getTasimaFiyati(sefer) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__8722cc04._.js.map